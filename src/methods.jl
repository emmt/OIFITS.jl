...
Base.eltype(::Union{T,Type{T}}) where {S,T<:OIDataBlock{S}} = S
...
# extend push! for OIArray, OIWavelength, and OICorr.
...
function load!(dest::Vector{OIDataBlock{T}}, hdu::TableHDU) where {T<:AbstractFloat}
    extn = read_keyword(String, hdu, "EXTNAME")
    if extn !== nothing
        if extn == "OI_TARGET"
            push!(dest, _read(OITarget{T}, hdu))
